...
function showRules() {
  ...
  changeVisibility([slapText], 'hide');
};
...
function startGame() {
  ...
  toggleButtons(); changeVisibility([slapText], 'hide'); changeVisibility([player1Hand, player2Hand], 'show');
  rulesControlsButton.classList.add('hidden');
  newGame.dealHands();
};
...
function changeMiddleCard(deck, deckContainer) {
  ...
  `<img class="middle-deck" src="assets/${topCard}.png" alt="${topCard}"></img><p class="visibility-hidden">Player</p><p class="visibility-hidden">wins</p>`;
};
